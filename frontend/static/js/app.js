...
function applyConfig() {
  ...
  const ghUrl = ft.github_url || "https://github.com/pitchile/looking-glass";

  // GitHub - header, sub-header, footer
  if (ft.show_github) {
    ["hdr-github","sub-github","footer-github"].forEach(id => {
      const el = document.getElementById(id);
      if (el) { el.href = ghUrl; el.style.display = ""; }
    });
  }

  // LATAM IXPs tab
  const tabLatam = document.getElementById("tab-latam");
  if (tabLatam && ft.show_latam_ranking !== false) tabLatam.style.display = "";

  // Cloud Status tab
  const tabCloud = document.getElementById("tab-cloud");
  if (tabCloud && ft.show_cloud_status !== false) tabCloud.style.display = "";
...
async function loadCloudStatus() {
  ...
  const tsEl = document.getElementById("cloud-ts");

  try {
    if (grid) grid.innerHTML = '<div class="loading-cell" style="grid-column:1/-1;">Consultando status de cloud providers...</div>';
    ...
    data.providers.forEach(p => {
      const ind = p.indicator || "unknown";
      const badgeCls =
        ind === "none"     ? "ok"       :
        ind === "minor"    ? "warning"  :
        ind === "major"    ? "critical" :
        ind === "critical" ? "critical" : "unknown";

      const badgeTxt =
        ind === "none"     ? "Operational" :
        ind === "minor"    ? "Degraded"    :
        ind === "major"    ? "Outage"      :
        ind === "critical" ? "Critical"    : "Unknown";

      const pitBadge = p.pit_maps_url
        ? `<span class="cloud-pit-badge">MAPS</span>`
        : "";
      ...
      div.innerHTML = `
        <div class="cloud-header">
          <div class="cloud-dot" style="background:${esc(p.color)};"></div>
          <span class="cloud-name">${esc(p.name)} ${pitBadge}</span>
          <span class="cloud-asn">AS${esc(p.asn)}</span>
        </div>
        <div class="cloud-status-row">
          <span class="cloud-status-badge ${badgeCls}">${badgeTxt}</span>
          <span class="cloud-desc">${esc(p.description || "")}</span>
        </div>
        <div class="cloud-links">
          <a href="${esc(p.page_url)}" target="_blank" class="cloud-link">Status page</a>
          <a href="#" class="cloud-link" onclick="event.preventDefault();
            document.getElementById('sel-command').value='bgp_aspath';
            document.getElementById('inp-bgp').value='${esc(p.asn)}';
            onCommandChange();
            switchTool('bgp');">
            BGP AS${esc(p.asn)}
          </a>
          ${p.pit_maps_url ? `<a href="${esc(p.pit_maps_url)}" target="_blank" class="cloud-link">PIT MAPS</a>` : ""}
        </div>`;
      grid.appendChild(div);
    });

    if (tsEl) {
      const ts = new Date(data.cached_at).toLocaleString("es-CL", {timeZone:"America/Santiago"});
      tsEl.textContent = "Actualizado: " + ts;
    }

    cloudLoaded = true;

  } catch(e) {
    if (grid) grid.innerHTML = `<div class="loading-cell" style="grid-column:1/-1;color:#c62828;">Error: ${esc(e.message)}</div>`;
  }
}
